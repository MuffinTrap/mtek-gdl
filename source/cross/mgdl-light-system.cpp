...
// NOTE for debugging
static void draw_cross(Vector3 position, Vector3 right, Vector3 up, float size)
{
    glBegin(GL_LINES);
    // Right
...
{
    // Default values
    params->sun_light_direction = Vector3New(0.0f, 0.0f, 1.0f); /**< Direction of light */
    params->sun_color = Vector3New(1.0f, 1.0f, 0.8f);
...
static inline void vertex(Vector3* center, Vector3* offset, float scale)
{
...
{
    // World axis vectors
    Vector3 w_right = Vector3New(1.0f, 0.0f, 0.0f);
    Vector3 w_up = Vector3New(0.0f, 1.0f, 0.0f);
    Vector3 w_forward = Vector3New(0.0f, 0.0f, 1.0f);
    Vector3 zero = Vector3New(0.0f, 0.0f, 0.0f);
    ...
    Vector3 to_light; V3f_Negate(params->sun_light_direction, to_light);
