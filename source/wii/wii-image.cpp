...
void gdl::ImageWii::_PrepareImage(short xRes, short yRes) {
...
bool gdl::ImageWii::_ProcessImage(png_structp png_ptr, u_int minFilt, u_int magFilt, u_int format, bool mipmapped, short maxMips) {
...
bool gdl::ImageWii::Create(short xRes, short yRes, u_int filterMode, u_int format) {
...
bool gdl::ImageWii::CreateMipmapped(short xRes, short yRes, u_int minFilt, u_int magFilt, short maxMipmaps, u_int format) {

	if (!gdl::ImageWii::Texture.CreateMipmapped(4*((xRes+3)/4), 4*((yRes+3)/4), minFilt, magFilt, maxMipmaps, format))
		return(false);
...
bool gdl::ImageWii::LoadImage(const char *fileName, u_int filterMode, u_int format) {
	...
	if (!gdl::ImageWii::_ProcessImage(png_ptr, 0, filterMode, format, false, 0)) {
		fclose(fp);
		return(false);
	}
...
bool gdl::ImageWii::LoadImageMipmapped(const char *fileName, u_int minFilt, u_int magFilt, short maxMipmaps, u_int format) {
	...
	if (!gdl::ImageWii::_ProcessImage(png_ptr, minFilt, magFilt, format, true, maxMipmaps)) {
		fclose(fp);
		return(false);
	}
...
bool gdl::ImageWii::LoadImageMem(void *imagePtr, u_int filterMode, u_int format) {
...
*/
bool gdl::ImageWii::LoadImageBuffer(const void *buffer, size_t size, u_int filterMode, u_int format) {
	if (gdl::ConsoleActive)
