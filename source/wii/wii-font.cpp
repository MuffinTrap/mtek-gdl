...
void gdl::FFont::BindSheet(ImageWii& image, short charw, short charh,  char firstCharacter) {
	...
	mgdl_assert_printf(image.Texture.TXsize()%charw == 0, "BindSheet error: %u mod %d not zero");
	mgdl_assert_printf(image.Texture.TYsize()%charh == 0, "BindSheet error: %u mod %d not zero");
	...
	if (vList == NULL)
	{
		...
		mgdl_assert_printf(vList != nullptr, "Out of memory when allocating font vertex list");
...
{
	mgdl_assert_print(cw > 0 && ch > 0, "Character dimensions not set");
	...
	if (tList == NULL)
	{
		...
		mgdl_assert_print(tList != nullptr, "Out of memory when allocation font txcord list");
	}
